...
static inline long double log_time(void)
{
  ...
  const long double elapsed = (current.tv_sec - start_time.tv_sec) + (current.tv_nsec - start_time.tv_nsec) / 1000000000.0L;
...
{
  const char* const restrict LEVEL_NAME[] = {"ERROR  ","WARNING","INFO   ","DEBUG  "};

  #ifndef LOGFILE
  const char* const restrict LEVEL_COLOR[] = {"\e[0;31m","\e[0;33m","\e[0;34m","\e[0;32m"};
  #endif
